...
```{r}
# Scalars are just vectors of lenght one
```
...
options(width = 110)
# When we attempt to combine different types they will be coerced to the most flexible type. Types from least to most flexible are: logical, integer, double and character.
...
```{r}
# Answer
# X will combine seveal lists into one. Given a combination of atomic vectors and lists, y will coerce the vectors to lists before combining them.
str(x)
str(y)
```
...
to get the first four letters. Check their types. What is the outcome? Where do you think
...
Finally coerce the list to a data.frame with
we fix it? What happend to the names?
as.data.frame() .
Why does it fail and how can
